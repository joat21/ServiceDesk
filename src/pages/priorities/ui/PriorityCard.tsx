...
import {
  PRIORITY_CHIP_MAP,
  PRIORITY_INDICATOR_COLOR,
  type Priority,
} from '@/entities/priority';
import { AlertCircleIcon } from '@/shared/ui/icons';

export const PriorityCard: FC<Priority> = ({ name, sla }) => {
  return (
    <Card className="gap-5 px-4 py-6 w-full">
      <div className="flex justify-between items-center gap-2 pl-4 pb-2.5 border-b border-[#c3c0c0]">
        <div className="flex flex-col">
          <span>{PRIORITY_CHIP_MAP[name].label}</span>
          <span className="text-[#666]">Приоритет</span>
        </div>
        <AlertCircleIcon className={PRIORITY_INDICATOR_COLOR[name]} />
      </div>
